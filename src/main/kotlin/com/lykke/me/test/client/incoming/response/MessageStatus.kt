...
enum class MessageStatus(val type: Int) {
    ...
    TOO_HIGH_MID_PRICE_DEVIATION(437),
    NEGATIVE_OVERDRAFT_LIMIT(433)
    ;

    companion object {
        private val valuesByType = MessageStatus.values()
                .groupBy { it.type }
                .mapValues { it.value.single() }

        fun getByType(type: Int): MessageStatus {
            return valuesByType[type]
                    ?: throw IllegalArgumentException("MessageStatus (type=$type) is not found")
        }
    }
